...
var Table = function () {
    ...
    _createClass(Table, [{
        ...
        key: 'css',
        value: function css() {
            return '<link href="/static/css/student_download.css" rel=\'stylesheet\' type=\'text/css\' />' +
                '<style>.table-wrapper label{text-align: right;display: block;font-size: 20px;margin-bottom: 3px;} .table-wrapper label select{padding: 0px 8px 3px;font-size: 15px;margin-right: 17px;cursor: pointer;height: 30px}.table-wrapper label input[name=T-search]{color:white;font-size: 18px;height: 25px;background: rgba(0, 0, 0, 0.32);margin-right: 17px}' +
                '.table-wrapper label .btn{color: #ffffff;\n' +
                '            font-weight: 100;\n' +
                '            padding: 0.4em 0;\n' +
                '            font-size: 15px;\n' +
                '            font-weight: 400;\n' +
                ...
                '            margin-right: 17px}' +
                '.table-wrapper label .btn:hover{ ' +
                '            color: #fff;\n' +
                '            box-shadow: 0 20px 5px -10px rgba(0, 0, 0, 0.4);\n' +
                '            transform: translateY(5px);} ' +
                '.table-wrapper #T-page ul{\n' +
                '    list-style: none;float: right;\n' +
                '    display: inline-block;\n' +
                '    -webkit-user-select: none;\n' +
                '    -ms-user-select: none;\n' +
                '    user-select: none;\n' +
                '    \\xA0-moz-user-select:none;\n' +
                '}\n' +
                '.table-wrapper #T-page ul li{\n' +
                '    display: inline;\n' +
                '}\n' +
                '.table-wrapper #T-page ul li a{\n' +
                '    text-align: center;\n' +
                '    padding:2px 10px;\n' +
                ...
                '    font-size: 20px;\n' +
                '}\n' +
                '.table-wrapper #T-page ul li.none{\n' +
                '    display: none;}\n' +
                '.table-wrapper #T-page ul li.directing{\n' +
                '    margin: 0 10px;}\n' +
                '.table-wrapper #T-page ul li a.not_allow{\n' +
                '    cursor: not-allowed;\n' +
                '    pointer-events: none;\n' +
                '    color: #777;}\n' +
                '.table-wrapper #T-page ul li a.active{\n' +
                '    background-color: #819ee9;\n' +
                '    color: white;\n' +
                '    pointer-events: none;\n' +
                '    cursor: not-allowed;}\n' +
                '.table-wrapper #T-page ul li a:hover:not(.active){\n' +
                '    background-color:  #dce6ff;}</style>';
        }
    }, {
        key: 'tableHtml',
        value: function tableHtml() {
            ...
            var container = ['<h2 style="position: relative;top:150px;font-size: 30px;color: black; ">' + title + '</h2>','<div class="table-wrapper">', '<table class="fl-table" border="0" cellspacing="0" cellpadding="0">',
                '<label for="T-sort">','\u6BCF\u9875\u663E\u793A\u6761\u6570\uFF1A<select name="" id="T-pageNum" style="top: -2px;">'].concat(_toConsumableArray([5, 10, 15].map(function (c) {
                return '<option value="' + c + '">' + c + '</option>';
            })), ['</select>', '\u6392\u5E8F\uFF1A<select name="" id="T-sort" style="top: -2px">'], _toConsumableArray(columns.map(function (c) {
                return '<option value="' + c.data + '">' + c.title + '</option>';
            })), ['</select><input type="text" name="T-search" placeholder="\u641C\u7D22"><input type="button" value="添加" class="btn" name="btn" onClick="msgbox(1)"/><a class="btn" href="/teacher/exportstudents" style="margin-right: 0">导出</a></label>'], _container, ['<div id="T-page"></div>', '</div>', '</div>']).join('');
...
function msgbox(n){
    document.getElementById('box').style.display=n?'block':'none';     /* 点击按钮打开/关闭 对话框 */
    document.getElementById('root').style.display=n?'none':'block';
}

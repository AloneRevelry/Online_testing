...
var Table = function () {
    ...
    _createClass(Table, [{
        ...
        value: function tableHtml() {
            ...
            })), ['</select><input type="text" name="T-search" placeholder="\u641C\u7D22"><input type="button" value="关闭" class="btn" onClick="msgbox(0)"/></label>'], _container, ['<div id="T-page"></div>', '</div>', '</div>']).join('');
...
function msgbox(n){
    document.getElementById('detil_info').style.display=n?'block':'none';     /* 点击按钮打开/关闭 对话框 */
    document.getElementById('info').style.display=n?'none':'block';
}
